...
/* plaja add-on */
#include "../../search/fd_adaptions/timer.h" // for plaja timeout handling
...
void signal_handler(int signal_number) {
    /*plaja add-on*/
    std::unique_lock lock(PLAJA_GLOBAL::signalLock);
    if (PLAJA_GLOBAL::timer) { PLAJA_GLOBAL::timer->pop_all(); } // properly handle time stats
    if (PLAJA_GLOBAL::currentEngine) {
        PLAJA_GLOBAL::currentEngine->signal_handling();
        PLAJA_GLOBAL::currentEngine = nullptr; // to suppress subsequent handles; engine is invalidated now
    }
    else { std::cout << "Signal handling: no engine to handle ..." << std::endl; }
    /*plaja add-on end*/
    print_peak_memory_reentrant();
    ...
    raise(signal_number);
    signal(signal_number, SIG_DFL); // plaja: re-added to allow for termination via terminal
}
