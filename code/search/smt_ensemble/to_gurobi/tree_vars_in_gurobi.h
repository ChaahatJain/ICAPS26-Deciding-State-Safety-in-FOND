...
#ifndef PLAJA_TREE_VARS_IN_GUROBI_H
#define PLAJA_TREE_VARS_IN_GUROBI_H
...
extern const std::string leafPrefix;
extern const std::string predicatePrefix;
extern const std::string outputPrefix;
extern const std::string featurePrefix;
extern const std::string treePrefix;

class VeritasToGurobiVars final {

private:
    std::shared_ptr<GRBModel> model;
    std::unordered_map<VeritasVarIndex_type, GRBVar> feature_mapping;
    std::map<std::tuple<int, int, int>, GRBVar> leavesToGurobi; // leaves variables - For every tree, we have tree, class, leaf index key
    std::map<std::tuple<int, int, int>, GRBVar> nodesToGurobi; // predicate variables - For every tree, we have tree, class, node index pair
...
public:
    VeritasToGurobiVars(std::shared_ptr<GRBModel> gurobi_model, const veritas::FlatBox& inputs, veritas::AddTree trees = veritas::AddTree(0, veritas::AddTreeType::REGR), int action_label = 0);
    ~VeritasToGurobiVars();
    DELETE_CONSTRUCTOR(VeritasToGurobiVars)

    [[nodiscard]] inline GRBModel& _model() const { return *model; }

    void add(VeritasVarIndex_type veritas_var, double lb, double ub);
    void add_leaf(int tree_index, int label, int leaf_index);
    void add_tree_action_aux(int tree_index, int action_label);
    void add_tree_action_aux_constraint(int tree_index, int label, veritas::Tree tree);
    void add_label_value(int action_label, int num_trees);
    void add_predicate(int tree_index, int label, int node_id, veritas::LtSplit split);
    void add_leaf_consistency(int tree_index, int label, veritas::Tree tree);
    void add_leaf_sum_consistency(int tree_index, int label, veritas::Tree tree);
    void add_leaf_root_consistency(int tree_index, int label, veritas::Tree tree, veritas::NodeId node);
    void add_leaf_internal_consistency(int tree_index, int label, veritas::Tree tree, veritas::NodeId node);
    ...
    [[nodiscard]] const GRBVar& leaf_to_gurobi(int tree_index, int label, int leaf) const {
        std::tuple<int, int, int> key = {tree_index, label, leaf};
        auto it = leavesToGurobi.find(key);
        return it->second; 
    }

    [[nodiscard]] const GRBVar& node_to_gurobi(int tree_index, int label, int node_id) const {
        std::tuple<int, int, int> key = {tree_index, label, node_id};
...
#endif //PLAJA_TREE_VARS_IN_GUROBI_H
